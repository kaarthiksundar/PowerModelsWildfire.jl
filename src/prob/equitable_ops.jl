...
""
function run_equitable_ops(file, model_constructor, optimizer; kwargs...)
    return _PM.solve_model(file, model_constructor, optimizer, build_equitable_ops;
        ref_extensions=[_PM.ref_add_on_off_va_bounds!], kwargs...)
end


function build_equitable_ops(pm::_PM.AbstractPowerModel)
    ...
    log_load = _PM.var(pm, nw_id_default, :log_load)
